<template>
    <div>
                  ...
                  <ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
                    ...
                    <li class="nav-item dropdown">
                      ...
                      <ul class="dropdown-menu dropdown-menu-dark">
                        ...
                        <router-link to="/section_edit" class="dropdown-item active">Edit Section</router-link>
      ...
      <div class="mt-5 p-2 container toggle-hidden">
        <h1 id="heading" class="mt-5">Section Edit</h1>
        <div v-for="section in sections" :key="section.section_title">
          <form @submit.prevent="handleSubmit(section.section_title)" class="shadow-lg p-3 m-3 rounded-3 border">
            <h1>{{ section.section_title }}</h1>
            <h1 class="fs-2 text-secondary">{{ section.section_description }}</h1>
            <label class="form-label" for="new_description">Enter New Description:</label>
            <input v-model="newDescription[section.section_title]" class="form-control" type="text" name="new_description" id="new_description" required>
            ...
            <button type="submit" class="m-3 btn btn-primary">Edit</button>
          </form>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        name: this.props_adminname || '', // Placeholder for user name
    ...
    mounted() {
      // Fetch user data and section data from backend
      this.fetchUserData();
    },
    methods: {
      async fetchUserData() {
      ...
      async handleSubmit(sectionTitle) {
        try {
          const newDescription = this.newDescription[sectionTitle];
          ...
          if (response.ok) {
            ...
            console.log(`Section ${sectionTitle} edited successfully.`);
            this.$router.push("/libr_dash");
            // Redirect or perform any other actions as needed
          } else {
            console.error('Failed to edit section:', response.statusText);
          }
        } catch (error) {
          console.error('An error occurred while editing section:', error);
        }
      }
    }
  };
